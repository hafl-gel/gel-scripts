...
read.GF3 <- function(Folder="",From=NULL,To=NULL,tz="Etc/GMT-1",DTA_only=TRUE,asCharacter=FALSE,time_add=0){
	...
	if((From_null <- is.null(From)) & (To_null <- is.null(To))){
		...
		for(Yr in Years){
			...
			Files <- c(Files,paste(Folder_Yr,Dir_Files[Dir_Files_Date>=From_date & Dir_Files_Date<=To],sep="/"))
		}
	}
	...
	if(DTA_only){
		...
		for(i in seq_along(Files)){
			DataList[[i]] <- fread(cmd = paste0("grep -e '$GFDTA' '",path.expand(Files[i]),"'"),fill=TRUE,blank.lines.skip = TRUE)
		}
		...
		DataList <- vector("list",length(Files))
		for(i in seq_along(Files)){
			DataList[[i]] <- fread(file = Files[i],fill=TRUE,blank.lines.skip = TRUE)
		}
		Data <- rbindlist(DataList)
		rm(DataList)

		#### DTA
		DTA <- Data[grepl("$GFDTA",V1,fixed = TRUE),V1:V8]
		DTA[,V8 := gsub("[*].*$","",V8)][,V1 := NULL]
		setnames(DTA,c("CH4 (ppm-m)","R2","distance","received_power","date_time","ser_number","status_code"))

		dateTime <- DTA[,parse_date_time(date_time,orders="%Y/%m/%d %H:%M:%S",tz=tz)] + time_add
		st_dateTime <- dateTime[1]
		TimeDiff <- diff(dateTime) 
		dT0 <- which(TimeDiff==0)
		if(length(dT0)){
		...
		stTime <- dateTime[takeme]
		# browser()
		nms <- names(DTA)[!(names(DTA) %in% "date_time")]
		Out_DTA <- as.ibts(DTA[(takeme),nms,with=FALSE],st=stTime,et=stTime + c(pmin(TimeDiff[TimeDiff!=0],3),median(TimeDiff[TimeDiff!=0])))
		if(!asCharacter){
			for(i in 1:4)Out_DTA[,i] <- as.numeric(Out_DTA[,i])
			colClasses(Out_DTA)[1:4] <- c("avg","min","avg","min")
		}

		#### LCA
		LCA <- Data[grepl("$GFLCA",V1,fixed = TRUE),]
		...
		#### DBG
		DBG <- Data[grepl("$GFDBG",V1,fixed = TRUE),]
		...
		# stop("To Do!")
		Out <- structure(Out_DTA,LCA=LCA,DBG=DBG)
	}

	if(any(From_null,To_null)){
		Out
	} else {
		Out[deparse_timerange(From,To)]
	}
}
...
correct.pressure <- function(P,a,b){
	a*(4*P^2/b - sqrt(2)*P^2/b * ((b/P^2 + 2) * sqrt(2 + 2*b/P^2) / (1 + b/P^2)))^-1
}
...
options(gf.calibration = list(
	...
	,"CH4OP.30015" = list(p = c(-0.3130739872,21583.3936240731),T = c(9.225871e-01,3.707545e-03,8.155414e-06,-1.531357e-09,2.666446e-11,-1.327185e-13,8.755430e-17,1.190408e-18))
	,"CH4OP.30016" = list(p = c(-0.2886017923,16039.7106451184),T = c(9.327787e-01,3.217314e-03,7.313662e-06,-3.882585e-09,2.951756e-11,-8.829886e-14,4.191947e-17,2.047603e-19))
	,"CH4OP.30017" = list(p = c(-0.3130739872,21583.3936240731),T = c(9.225871e-01,3.707545e-03,8.155414e-06,-1.531357e-09,2.666446e-11,-1.327185e-13,8.755430e-17,1.190408e-18))
	,"CH4OP.30018" = list(p = c(-0.3177601163,23100.0009499224),T = c(9.202980e-01,3.817144e-03,8.401504e-06,-1.683137e-09,1.661059e-11,9.442030e-14,-1.414051e-15,4.508541e-18))
	))

C_pT <- function(instrument,p_hPa,T_deg,p_hPa.inst=1013.25,T_deg.inst=20,units=c("mgm3","ppm")){
	# get instrument's calibration
	CalPars <- getOption("gf.calibration")[[switch(which(sapply(c("1003","1049","15","16","17","18"),function(x)grepl(x,instrument)))
		,"CH4OP.1003"
		,"CH4OP.1049"
		,"CH4OP.30015"
		,"CH4OP.30016"
		,"CH4OP.30017"
		,"CH4OP.30018"
		,stop("Could not relate argument 'instrument' to any available instrument ('1003','1049','30015','30016','30017','30018')")
		)]]
	# convert raw data to ppm @ 20° and 1 atm
	...
	# convert ppm @ 20° and 1 atm to chosen unit
	switch(pmatch(units[1],c("ppm","ppb","mgm3","ugm3"))
		# "ppm"
		, C_out*1013.25/293.15*(T_deg + 273.15)/p_hPa
		# "ppb"
		, C_out*1013.25/293.15*(T_deg + 273.15)/p_hPa*1000
		# "mgm3"
		, C_out*16.04*101.325/(8.3144598*293.15)
		# "ugm3"
		, C_out*16.04*101325/(8.3144598*293.15)
		# else
